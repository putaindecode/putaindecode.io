...
Cependant NPM ne se limite pas qu'aux modules JavaScript. Il est en effet possible
...
`package.json` (fichier de configuration utilisé par NPM). Le champs
...
C'est grâce au champs "[files](https://www.npmjs.org/doc/json.html#files)" que
...
- Mettre à jour le champs "version" de votre `package.json` (vous pouvez vous
...
NPM est une excellente solution pour gérer les dependences de votre projet, aussi
