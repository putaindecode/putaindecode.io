Nous vivons aujourd'hui dans un monde où tout devient service. Twitter, Facebook, Google Drive, Github, tout le monde y va de son petit API pour que nous, geeks, puissions profiter pleinement des moultes fonctionnalités offertes par ces plateformes. Qu'entend-on par service ? Comment les rendre disponibles facilement et intelligemment ? On va essayer d'y répondre ! 
...
3. Le navigateur reçoit du code HTML et l'interprête, pour voir si il faut charger d'autres choses
...
Il y a un type qui s'appelle Leonard Richardson et qui a mis en place un [modèle de maturité](http://martinfowler.com/articles/richardsonMaturityModel.html) pour qualifier les APIs webservice.
...
De la même manière, tu auras remarqué que l'on ne précise pas le verbe HTTP à utiliser sur les URLs. C'est la raison pour laquelle il faut implémenter un retour cohérent pour tous les verbes HTTP courants !
