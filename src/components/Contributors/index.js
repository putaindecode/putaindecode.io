...
const getGithubUrl = (repo, action, filename) =>
  `${repo}/${action}/master/content/${filename}`;
...
const Contributors = ({ filename, reviewers }, context) => {
  ...
  const fileContributors =
    (contributors.files && contributors.files[`content/${filename}`]) || [];
      ...
      <div className="r-Grid putainde-Contributors-actions">
        <div className="r-Grid-cell r-all--1of3">
          <a
            ...
            href={getGithubUrl(httpRepository, "edit", filename)}
          >
            {i18n.pageEdit}
          </a>
        </div>
        <div className="r-Grid-cell r-all--1of3">
          <a
            className="putainde-Contributors-action"
            href={getGithubUrl(httpRepository, "blame", filename)}
          >
            {i18n.pageBlame}
          </a>
        </div>
        <div className="r-Grid-cell r-all--1of3">
          <a
            className="putainde-Contributors-action"
            href={getGithubUrl(httpRepository, "commits", filename)}
          >
            {i18n.pageHistory}
          </a>
        </div>
