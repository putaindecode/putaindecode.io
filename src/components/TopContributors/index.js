...
const TopContributors = ({ recentContributions }, context) => {
  ...
  recentContributors.sort(
    (a, b) => recentContributions[b] - recentContributions[a]
  );
  ...
  return (
    <div>
      <div className="putainde-Title putainde-Title--home">
        <h2 className="putainde-Title-text">{i18n.topContributors}</h2>
        <small style={{ opacity: 0.5 }}>{i18n.topContributorsNote}</small>
      </div>
      ...
      >
        {!topContributors.length && (
          <p className="r-Grid-cell" style={{ opacity: 0.5 }}>
        ...
        {topContributors.length &&
          topContributors.map(author => {
            return (
              <div
                key={author}
                className={"r-Grid-cell r-all--1of4 r-minM--1of8"}
              >
                <Contributor
                  author={author}
