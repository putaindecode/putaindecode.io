...
const Author = ({ afterName, author, bio, className, isPost }, context) => {
  ...
  return (
    <div className={`putainde-Author ${className}`}>
      <Avatar author={authorData.login} className="putainde-Author-picture" />

      <div className="putainde-Author-description">
        <div className="putainde-Author-title">
          <h3 className="putainde-Author-name">
            {isPost && (
              <span className="putainde-WrittenBy">{`${i18n.writtenBy} `}</span>
            )}
            <a className="putainde-Link" href={getAuthorUri(authorData)}>
              {authorData.login}
            </a>
            {afterName && (
