...
publics, a moins de souscrire pour un compte privé payant, donc n'y stockez rien
de sensible/secret.

## Qu'est-ce qu'un site web statique ?

Pour la plupart des sites, les solutions de Blog, CMS... type WordPress & autres
sont largement surdimensionnées, requièrent un hébergement particulier (PHP ou
autre), des connaissances techniques pour l'installation, et surtout une
maintenance à moyen/long-terme à cause des mises à jour surprises de sécurité
et corrections de bugs à appliquer.

Or, il est possible de créer des sites webs modernes et performants qui n'ont
pas besoin de code coté serveur (donc pas de PHP), et peuvent donc être hebergés
n'importe où, et à moindre coût. Le code du site sera uniquement composé de
HTML, CSS et Javascript, et s'executera directement dans le navigateur du
...
des fonctionnalités tierces, sans avoir besoin de code coté serveur, donc cette
...
[https://github.com/new](https://github.com/new) ex: supersite

Le repository sera crée à cette adresse :
...
Allez sur la page du projet :  [https://github.com/USERNAME/supersite](https://github.com/USERNAME/supersite)
 ...
 - Editez le texte du site
 - Selectionnez votre template préféré puis **Publish Page**
...
en-dessous, avec un titre explicite permettant d'identifier votre modificaiton.

exemple : "ajout des infos de contact".

Cliquez **Commit changes**, attendez quelques secondes et allez sur
[http://USERNAME.github.io/supersite](http://USERNAME.github.io/supersite)
pour admirer les changements.

**✔** Site web mis à jour :)

## Editer le site sur son ordinateur
...
ou [windows.github.com](http://windows.github.com)
 ...
 - **Clone** : va rappatrier les fichiers de votre site sur votre ordinateur
 - **Editez** les fichiers directement
 - **Testez** si le site fonctionne comme prévu
 - Puis choisisez **Commit** pour enregistrer vos modifications
...
(où se trouve index.html) puis ouvez http://127.0.0.1:8000 dans votre navigateur.
(Ceci est du à une [restriction de sécurité](https://developer.mozilla.org/fr/docs/Web/JavaScript/Same_origin_policy_for_JavaScript))


## Installer un superbe template

Si vous voulez des templates différents de ceux proposés par Github.com,
vous avez simplement besoin de remplacer les fichiers présents dans le dossier
du projet.

Choisissez par exemple un des templates gratuits de haute qualité disponible sur
[html5up.net](http://html5up.net) ou sur [pixelarity.com](http://pixelarity.com)
(payant).

Téléchargez les sources et placez les fichiers dans votre projet.

Editez les fichiers, testez, puis commit & sync, et voilà :)

## Et après ?

Apprenez la syntaxe Markdown : [reference](https://en.support.wordpress.com/markdown-quick-reference/)
...
### Comment installer un nom de domaine ?
 ...
 - Achetez un nom de domaine sur [gandi.net](http://gandi.net) ou autre
fournisseur sérieux (surtout pas chez pas 1&1, lws ou autre discounter douteux)
 - Editez la "Zone DNS" du domaine et suivez les
...
Aide Github :  [custom domain](https://help.github.com/articles/setting-up-a-custom-domain-with-github-pages/), [dns setup](https://help.github.com/articles/tips-for-configuring-a-cname-record-with-your-dns-provider/)

### Comment ajouter une page ?

Pour ajouter simplement des pages supplémentaires, ajoutez simplement un
nouveau fichier HTML, par exemple `produits.html` dans le projet et ajoutez un
lien vers cette page depuis l'index, par exemple
`<a href="produits.html">consulter les produits</a>`.

### Comment analyser le traffic de mon site ?

Si vous voulez tracker vos visiteurs, savoir d'où ils viennent, ce qu'ils font
et où ils vont, créez un compte sur
[Google Analytics](http://www.google.com/analytics) et ajoutez le code de tracking sur vos pages HTML.
...
Créer un compte sur [stripe.com](stripe.com) et ajoutez le code javascript sur
votre page.

### Qu'est-ce que gh-pages ?

`gh-pages` est une branche spéciale de votre code sur github (une version du
    code) qui publie et hébèrge automagiquement votre site.
...
visiteurs et vous ferons monter progressivment dans les résultats au fil des mois.



Voilà pour ce premier pas vers le développement web !

Les commentaires ci-desous sont là pour recevoir vos remarques et questions :)
