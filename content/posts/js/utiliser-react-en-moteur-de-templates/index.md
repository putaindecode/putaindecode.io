...
Sa syntaxe plutôt est libératrice.
...
certain. Les méthodes de rendu de React (ce qui va nous intéressé) est très proche
de l'HTML.

Ceux qui on utilisé JSX (bien qu'ayant pu avoir des nausées au premier contact)
avouent souvent qu'on fini par l'aimer cette petite syntaxe créé par Facebook qui
fonctionne très bien avec React.

Du coup, si on utilisait ça pour faire de simple templates et non pas des composants
d'interface utilisateur dynamiques ? Qui peut le plus peut le moins (quoi que en
...
Nous n'aborderons pas dans le détail de la syntaxe React/JSX et ES6 pour rester
focalisé sur la finalité (on a prévu des articles à ce propos, en attendant
...
Si dessous vous trouverez un exemple plus complexe avec une page pour la liste
des posts et quelques composants.
Les eléments qui ressemble à de HTML qui commence par une majuscule sont des
composants qui doivent être défini (dans notre cas importé), c'est une particularité
de React.
...
Voici deux exemples de composant utilisés ci-dessus
...
pour générer un site statique, et c'est donc sur ma machine quelconque que la
génération se fait).

Voilà de quoi rentre en bon vieux HTML nos composants (car c'est la finalité
...
Pour aller plus loin on pourrait décidé d'utiliser `renderToString()` plutôt que
